        ...
        content: SingleChildScrollView(
          child: Column(
            ...
            children: [
              ...
              const Text('版本 1.3.0', style: TextStyle(fontSize: 13, color: Colors.grey)),
    ...
    return Scaffold(
      ...
      body: ListView(
        children: [
          ...
          Padding(
            ...
            child: Column(
              ...
              children: [
                Row(
                  children: [
                    const Icon(Icons.inventory_2_outlined, size: 20),                    const SizedBox(width: 12),
                ...
                const Padding(
                  ...
                  child: Text('留空使用默认源', style: TextStyle(fontSize: 12, color: Colors.grey)),
                ),
          ...
          Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                const Padding(
                  padding: EdgeInsets.only(left: 32),
                  child: Text('脚本运行时的文件读写目录，留空使用默认', style: TextStyle(fontSize: 12, color: Colors.grey)),
                ),
                const SizedBox(height: 8),
                Row(
                  children: [
          ...
          Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                const Padding(
                  padding: EdgeInsets.only(left: 32),
                  child: Text('留空使用默认下载目录', style: TextStyle(fontSize: 12, color: Colors.grey)),
                ),
                const SizedBox(height: 8),
                Row(
                  children: [
            ...
            child: Column(
              ...
              children: [
                Row(children: [
                  ...
                  const SizedBox(width: 12),
                  Text('网络调试模式', style: Theme.of(context).textTheme.titleSmall),
                ]),
                const SizedBox(height: 4),
                const Padding(
                  padding: EdgeInsets.only(left: 32),
                  child: Text('用于抓包/代理调试，默认关闭', style: TextStyle(fontSize: 12, color: Colors.grey)),
                ),
              ],
            ),
          ),
          ...
          // ── HTTP Request Recording & Override ──
          Padding(
            padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 12),
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Row(children: [
                  ...
                  const SizedBox(width: 12),
                  Text('网络请求调试', style: Theme.of(context).textTheme.titleSmall),
                ]),
                const SizedBox(height: 4),
                const Padding(
                  padding: EdgeInsets.only(left: 32),
                  child: Text('记录和覆盖 Python HTTP 请求行为', style: TextStyle(fontSize: 12, color: Colors.grey)),
                ),
              ],
            ),
          ),
            ...
            child: Column(
              ...
              children: [
                Row(children: [
                  ...
                  const SizedBox(width: 12),
                  Text('系统日志管理', style: Theme.of(context).textTheme.titleSmall),
                ]),
                const SizedBox(height: 4),
                const Padding(
                  padding: EdgeInsets.only(left: 32),
                  child: Text('查看、导出或清空系统运行日志', style: TextStyle(fontSize: 12, color: Colors.grey)),
                ),
              ],
            ),
          ),
        ...
        children: [
          _ManualSection(
            ...
            items: const [
              ...
              _ManualItem(Icons.more_vert, '长按脚本', '运行、重命名、复制、导出、删除'),
            ],
          ),
          ...
          _ManualSection(
            ...
            items: const [
              ...
              _ManualItem(Icons.play_arrow, '本地运行', '在本机 Python 环境运行'),
              _ManualItem(Icons.terminal, '输出面板', '显示/隐藏输出，可拖拽调整高度'),
              _ManualItem(Icons.fullscreen, '全屏输出', '输出面板展开为全屏'),
          ...
          _ManualSection(
            ...
            title: '图形引擎',
            items: const [
              _ManualItem(Icons.toggle_on, '启用方式', '设置页打开「图形引擎」开关'),
              _ManualItem(Icons.code, 'scene 模块', '兼容 Pythonista scene API，支持 Scene 子类'),
              _ManualItem(Icons.brush, '绘图函数', 'fill/rect/ellipse/line/text 等画布绑定'),
              _ManualItem(Icons.touch_app, '触摸交互', 'touch_began/moved/ended 接收屏幕触摸'),
              _ManualItem(Icons.speed, '游戏循环', '30fps 自动循环 update → draw'),
              _ManualItem(Icons.fullscreen, '自动全屏', '运行 scene 脚本时输出面板自动全屏'),
            ],
          ),
          const SizedBox(height: 12),
          _ManualSection(
            ...
            items: const [
              ...
              _ManualItem(Icons.filter_list, '筛选', '按域名、请求方法、状态码筛选请求'),
              _ManualItem(Icons.tune, '请求覆盖', '设置页开启后可全局覆盖 UA/Headers/Cookie'),
              _ManualItem(Icons.person, 'User-Agent', '覆盖 requests 等库的默认 UA，解决被网站拦截'),
              _ManualItem(Icons.code, '支持的库', 'requests、httpx、urllib3 自动 Hook'),
              _ManualItem(Icons.bug_report_outlined, '代理调试', '配合 Charles/Fiddler 外部抓包工具使用'),
              _ManualItem(Icons.lock_open, 'SSL 调试', '允许不安全证书，配合抓包工具的 MITM 证书'),
              _ManualItem(Icons.file_download_outlined, '导出记录', '复制或导出捕获的网络请求日志'),
            ],
          ),
          const SizedBox(height: 12),
          _ManualSection(
            ...
            title: '运行日志',
            items: const [
              _ManualItem(Icons.file_download_outlined, '导出日志', '保存到设备下载目录'),
              _ManualItem(Icons.copy, '复制日志', '单条或全部复制到剪贴板'),
              _ManualItem(Icons.delete_outline, '清除记录', '清除全部运行历史'),
              _ManualItem(Icons.touch_app, '查看详情', '点击单条查看完整日志'),
            ],
          ),
          const SizedBox(height: 12),
          _ManualSection(
